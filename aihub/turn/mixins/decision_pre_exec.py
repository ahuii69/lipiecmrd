...
def run_pre_exec_decision_core(self, *, turn: ChatTurnInput, ctx: ChatTurnContext, psyche_snapshot: dict[str, Any], memory_v2_runtime_ctx: Any=None, psyche_v2_behavior_ctx: Any=None) -> dict[str, Any]:
    ...
    try:
        ...
        _msg = turn.message or ''
        _meta_ask = is_assistant_meta_ask(_msg)
        _pure_meta = _meta_ask and not meta_ask_refers_to_prior_conversation(_msg)
        result['assistant_meta_ask'] = _meta_ask
        result['assistant_meta_ask_pure'] = _pure_meta
        try:
            _active_goals = [] if _meta_ask else get_goal_engine().get_actionable_goals(turn.user_id)
            if _active_goals:
                ...
                result['selected_goal'] = None
        except Exception:
            logger.debug('Decision core: active goals lookup failed', exc_info=True)
            _active_goals_summary = None
            result['selected_goal'] = None
        selection = select_strategy(user_id=turn.user_id, user_text=_msg, mode=ctx.mode, active_goals_summary=_active_goals_summary, history=list(turn.history or []), pragmatics=(ctx.system_context.get('pragmatics_obj') if isinstance(ctx.system_context, dict) else None))
        ...
        if _meta_ask:
            ...
            result['selected_goal'] = None
            result['web_decision'] = 'off'
            result['planner_recommended'] = False
            if result['selected_strategy'] not in ('instant', 'direct'):
                old = result['selected_strategy']
                result['selected_strategy'] = 'direct' if meta_ask_refers_to_prior_conversation(_msg) else 'instant'
                result['base_strategy'] = result['selected_strategy']
                result['strategy_adjustment_log'].append(
                    {
                        'source': 'meta_ask_guard',
                        'reason_code': 'META_ASK_FORCE_LIGHTWEIGHT',
                        'old_value': old,
                        'new_value': result['selected_strategy'],
        ...
        _meta_ask = False
        _pure_meta = False
    if not result.get('assistant_meta_ask'):
        try:
        ...
        result['experience_lookup_happened'] = False
        result['experience_matches_count'] = 0
        result['experience_influenced_strategy'] = False
        result['experience_signal_summary'] = 'skipped_meta_ask'
    if not result.get('assistant_meta_ask'):
        try:
        ...
        result['memory_influenced_strategy_chat'] = False
        result['psyche_influenced_strategy_chat'] = False
    if not result.get('assistant_meta_ask'):
        try:
        ...
        result['policy_hints_loaded'] = False
        result['policy_feedback_applied'] = False
        result['policy_hints'] = []
    ...
    if not result.get('assistant_meta_ask'):
        ...
        result['simulation_variants_count'] = 0
        result['simulation_best_action'] = None
        result['simulation_risk_summary'] = None
    if not result.get('assistant_meta_ask'):
        try:
        ...
        result['consistency_check_ran'] = False
        if isinstance(ctx.system_context, dict):
            ctx.system_context.pop('learning_decision', None)
            ctx.system_context.pop('knowledge_decision', None)
    self._local_non_research_guardrails(turn, result)
    # 20.07 / 24.07: hard clamp — identity/meta asks stay on lightweight path
    try:
        from aihub.strategy_selector import is_assistant_meta_ask, meta_ask_refers_to_prior_conversation

        if is_assistant_meta_ask(turn.message or ""):
            old = result.get("selected_strategy")
            target = (
                "direct"
                if meta_ask_refers_to_prior_conversation(turn.message or "")
                else "instant"
            )
            if old not in ("instant", "direct"):
                result["selected_strategy"] = target
                result["planner_recommended"] = False
                result["web_decision"] = "off"
                result["selected_goal"] = None
                ...
                result["selected_goal"] = None
                result["planner_recommended"] = False
                if result.get("web_decision") != "off":
                    result["web_decision"] = "off"
            # Strip junk goal reason if somehow present
            result["reason_codes"] = [
                c for c in list(result.get("reason_codes") or []) if c != "ACTIVE_GOAL_PRESENT"
            ]
    except Exception:
